...
(async function () {
  ...
  try {
    ...
    const migration = await DB.migrateCards();
    if (migration.migrated > 0) {
      console.log(`Migrated ${migration.migrated} of ${migration.total} cards to v2 schema.`);
    }
  ...
  async function initHome() {
    ...
    const streak = streakData ? (streakData.current || 0) : 0;
    ...
    homeView.querySelectorAll('[data-view]').forEach(btn => {
      btn.addEventListener('click', () => {
        navigateTo(btn.dataset.view);
      });
    });
  ...
  function showImportPreview(result) {
    ...
    // ──────────────────────────────────────────────
    // Categorize duplicates for display
    // ──────────────────────────────────────────────
    const reImportCount = result.duplicates.filter(d =>
      d.reason === 'same-id-exact' || d.reason === 'same-id-updated'
    ).length;
    const contentMatchCount = result.duplicates.filter(d =>
      d.reason === 'content-match-exact' || d.reason === 'content-match-different-answer'
    ).length;
    const mergeCount = result.duplicates.filter(d => d.action === 'merge').length;
    const skipCount = result.duplicates.filter(d => d.action === 'skip').length;
    ...
    mergeStats.innerHTML = `
      ...
      <div class="merge-stat"><span class="merge-stat-num" style="color:var(--accent-yellow)">${mergeCount}</span> merge</div>
      <div class="merge-stat"><span class="merge-stat-num" style="color:var(--text-muted)">${skipCount}</span> skip</div>
      <div class="merge-stat"><span class="merge-stat-num" style="color:var(--accent-red)">${result.conflicts.length}</span> conflicts</div>
    `;

    // ──────────────────────────────────────────────
    // Show detail breakdown when duplicates exist
    // ──────────────────────────────────────────────
    const mergeList = document.getElementById('mergeList');
    mergeList.innerHTML = '';

    if (reImportCount > 0 || contentMatchCount > 0) {
      const infoDiv = document.createElement('div');
      infoDiv.className = 'merge-item';
      infoDiv.style.cssText = 'font-size:0.8rem;color:var(--text-muted);padding:0.5rem 0;border-bottom:1px solid var(--bg-input)';
      let infoText = '';
      if (reImportCount > 0) infoText += `${reImportCount} already imported from this file. `;
      if (contentMatchCount > 0) infoText += `${contentMatchCount} matching term+domain found in other decks.`;
      infoDiv.textContent = infoText.trim();
      mergeList.appendChild(infoDiv);
    }

    result.newCards.slice(0, 20).forEach(card => {
      const div = document.createElement('div');
      div.className = 'merge-item';
      div.innerHTML = `<span class="merge-item-new">+ ${Utils.escapeHTML(card.term)}</span><span style="font-size:0.75rem;color:var(--text-muted)">${Utils.escapeHTML(card.domain)}</span>`;
    ...
    result.duplicates.forEach(dup => {
      ...
      const labelClass = dup.action === 'merge' ? 'merge-item-dup' : '';

      let sourceInfo = '';
      if (dup.existingCard && dup.existingCard.sourceDeck) {
        sourceInfo = ` <span style="font-size:0.7rem;color:var(--text-muted)">(in: ${Utils.escapeHTML(dup.existingCard.sourceDeck)})</span>`;
      }

      div.innerHTML = `<span class="${labelClass}">${actionText}: ${Utils.escapeHTML(dup.existingCard.term)}</span>${sourceInfo}`;
      mergeList.appendChild(div);
    });
