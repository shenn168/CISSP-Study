...
const DB = (() => {
  ...
  /**
   * Prepare a card for IndexedDB storage:
   * - Ensure a unique ID exists (UUID fallback)
   * - Flatten sm2.nextReviewDate for indexing
   * - Ensure all required array fields exist
   * - Preserve sourceId and sourceDeck metadata
   */
  function prepareCardForStorage(card) {
    // ──────────────────────────────────────────
    // Guarantee unique ID — if somehow missing,
    // generate a UUID so we never overwrite
    // another card by accident
    // ──────────────────────────────────────────
    if (!card.id) {
