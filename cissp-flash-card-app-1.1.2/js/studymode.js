/* ============================================================
   studymode.js — Classic Flip Card Study Mode (FIXED)
   ============================================================ */
...
const StudyMode = (() => {
  ...
  async function startSession() {
    ...
    // ──────────────────────────────────────────────
    // FIX 1: Read shuffle setting before shuffling
    // ──────────────────────────────────────────────
    const shuffleSetting = await DB.getSetting('shuffle');
    const shouldShuffle = shuffleSetting !== null ? shuffleSetting : true;
    ...
    // ──────────────────────────────────────────────
    // FIX 2: Read cardsPerSession and limit the queue
    // ──────────────────────────────────────────────
    const cardsPerSession = await DB.getSetting('cardsPerSession');
    if (cardsPerSession !== null && cardsPerSession > 0) {
      filtered = filtered.slice(0, cardsPerSession);
    }
    ...
    // ──────────────────────────────────────────────
    // FIX 2 (continued): Inform user of session limit
    // ──────────────────────────────────────────────
    if (cardsPerSession !== null && cardsPerSession > 0) {
      const totalAvailable = allCards.length;
      Utils.toast(
        `Session limited to ${currentQueue.length} of ${totalAvailable} available cards.`,
        'info'
      );
    }
  }
  ...
  // ──────────────────────────────────────────────
  // FIX: Rewritten to use Utils.dateDiffDays for
  // timezone-safe date comparison and track
  // todayCompleted flag
  // ──────────────────────────────────────────────
  async function updateStreak() {
    ...
    if (!streakData) {
      ...
      const diffDays = Utils.dateDiffDays(streakData.lastDate, today);
