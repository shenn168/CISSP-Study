...
(async function () {
  ...
  const themeToggleBtn = document.getElementById('popupThemeToggle');
  if (themeToggleBtn) {
  ...
  function isDueToday(card) {
    ...
    return card.sm2 && card.sm2.nextReviewDate && card.sm2.nextReviewDate <= today;
  }

  // ---- Helper to safely open StudyDeck ----
  function openStudyDeck(hash) {
    const url = hash ? 'newtab.html#' + hash : 'newtab.html';
  ...
  try {
    ...
    if (!body) {
      console.error('Popup error: #popupBody element not found');
      return;
    }
    ...
    if (openNewTabBtn) {
      openNewTabBtn.addEventListener('click', () => openStudyDeck(''));
    }

    const openStudyBtn = document.getElementById('openStudy');
    if (openStudyBtn) {
      openStudyBtn.addEventListener('click', () => openStudyDeck('study'));
    }

    const openDashboardBtn = document.getElementById('openDashboard');
    if (openDashboardBtn) {
      openDashboardBtn.addEventListener('click', () => openStudyDeck('dashboard'));
    }
    ...
    if (body) {
      ...
      `;
      const fallbackBtn = document.getElementById('openNewTabFallback');
      if (fallbackBtn) {
        fallbackBtn.addEventListener('click', () => openStudyDeck(''));
      }
    }
  }
