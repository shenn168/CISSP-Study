...
const Parsers = (() => {
  ...
  /**
   * Generate a deterministic but unique ID by combining the
   * source filename with the original card ID. This ensures:
   * - Same file re-imported → same generated IDs (merge detection works)
   * - Different files with same internal IDs → distinct keys (no overwrites)
   */
  function generateImportId(originalId, fileName) {
    var prefix = (fileName || 'unknown')
      .replace(/\.[^.]+$/, '')
      .replace(/[^a-zA-Z0-9_-]/g, '-')
      .substring(0, 50);
    return prefix + '__' + (originalId || Utils.generateUUID());
  }
...
')[0].toLowerCase();
  ...
  function parseJSON(content, fileName) {
    ...
    for (var index = 0; index < items.length; index++) {
      ...
      try {
        ...
        var cardData = normalizeJSONItem(item, fileName, index);
   ...
   */
  function normalizeJSONItem(item, fileName, index) {
    var data = {};

    // ──────────────────────────────────────────────
    // ID: Generate a unique composite ID from file + original ID.
    // Preserve the original for display as sourceId.
    // If the card was previously exported from this app with
    // a composite ID, detect and preserve it to avoid double-prefixing.
    // ──────────────────────────────────────────────
    var rawId = item.id || item.ID || item.cardId || ('IDX-' + ((index || 0) + 1));
    var alreadyComposite = typeof rawId === 'string' && rawId.indexOf('__') !== -1;

    if (alreadyComposite) {
      // Re-importing a previously exported card — keep existing composite ID
      data.id = rawId;
      data.sourceId = rawId.substring(rawId.indexOf('__') + 2);
    } else {
      data.id = generateImportId(rawId, fileName);
      data.sourceId = rawId;
    }

    data.sourceDeck = (fileName || 'Unknown').replace(/\.[^.]+$/, '');
    ...
    // Difficulty
    data.difficulty = item.difficulty || item.DIFFICULTY || item.Difficulty || 'beginner';
    ...
    if (item.primaryAnswer && typeof item.primaryAnswer === 'object') {
      ...
      data.primaryAnswer = {
        ...
        sourceDeck: item.primaryAnswer.sourceDeck || data.sourceDeck,
    ...
    if (Array.isArray(item.alsoSee)) {
      ...
      for (var a = 0; a < item.alsoSee.length; a++) {
        ...
        data.alsoSee.push({
          ...
          sourceDeck: entry.sourceDeck || data.sourceDeck,
  ...
  function parseCSV(content, fileName) {
    ...
    var colMap = {};
    var knownCols = ['term', 'domain', 'type', 'answer', 'examtip', 'tip', 'relatedcards', 'related', 'contributor', 'tags', 'scenariotext', 'scenario', 'flagged', 'id', 'difficulty'];
    ...
    for (var i = 1; i < lines.length; i++) {
      ...
      // Generate unique composite ID for CSV rows
      var rawCsvId = getCSVVal('id') || ('ROW-' + i);
      var csvCompositeId = generateImportId(rawCsvId, fileName);

      var cardData = {
        id: csvCompositeId,
        sourceId: rawCsvId,
        sourceDeck: (fileName || 'Unknown').replace(/\.[^.]+$/, ''),
        term: term,
        domain: getCSVVal('domain'),
        type: getCSVVal('type'),
        difficulty: getCSVVal('difficulty') || 'beginner',
      ...
      cardData.primaryAnswer = {
        ...
        sourceDeck: cardData.sourceDeck,
        importedAt: Utils.nowISO()
      };
...
');
      ...
      // Generate unique composite ID for text blocks
      var rawTxtId = fields.ID || ('TXT-' + (bi + 1));
      var txtCompositeId = generateImportId(rawTxtId, fileName);
  ...
  function matchFieldLabel(line) {
    var labels = ['TERM', 'DOMAIN', 'TYPE', 'ANSWER', 'TIP', 'RELATED', 'CONTRIBUTOR', 'TAGS', 'SCENARIO', 'FLAGGED', 'ID'];
